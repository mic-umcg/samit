...
%   Version 19.04
%       - The code was adjusted to allow the construction of a template without a reference MRI

%% Display
disp(' ');
disp('SAMIT: Construction of PET/SPECT brain template');
disp('-----------------------------------------------');
...
if isequal(template,0) || isequal(template_path,0)
    disp('Operation cancelled: Output file was not specified.');
    return
end
...
if isempty(files)
    disp('Operation cancelled: No files selected.');
    return
end
...
%% Adjust voxel size
M = spm_get_space(files(1,:));
M = spm_imatrix(M);
samit_def.normalise.write.vox = M(7:9);
...
if ~strcmp(atlas,'none') % B. If MRI available
