function samit_SPM2PMOD(files,d)
...
if d ~= false
    display(' ');
    display('SAMIT: Reorient images from SPM space to PMOD space');
    display('---------------------------------------------------');
end
...
for i = 1:numel(files)
    ...
    % Rotate matrix
    % SPM space to PMOD space (pitch rotation)
    theta = -pi/2;
    ...
    RA = imref3d(size(A),vox(1),vox(2),vox(3)); % Limits for rotation
